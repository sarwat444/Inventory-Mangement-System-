<script src="<?php echo $js ?>jquery-2.2.4.min.js"></script>
    <script src="<?php echo $js ?>popper.min.js"></script>
    <script src="<?php echo $js ?>bootstrap.min.js"></script>
    <script src="<?php echo $js ?>owl.carousel.min.js"></script>
    <script src="<?php echo $js ?>metisMenu.min.js"></script>
    <script src="<?php echo $js ?>jquery.slimscroll.min.js"></script>
    <script src="<?php echo $js ?>/jquery.slicknav.min.js"></script>
    
    <!-- start chart js -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.7.2/Chart.min.js"></script>
    <!-- start highcharts js -->
    <script src="https://code.highcharts.com/highcharts.js"></script>
    <!-- start zingchart js -->
    <script src="https://cdn.zingchart.com/zingchart.min.js"></script>
    <script>
    zingchart.MODULESDIR = "https://cdn.zingchart.com/modules/";
    ZC.LICENSE = ["569d52cefae586f634c54f86dc99e6a9", "ee6b7db5b51705a13dc2339db3edaf6d"];
    </script>
    <!-- start amchart js -->
    <script src="https://www.amcharts.com/lib/3/amcharts.js"></script>
    <script src="https://www.amcharts.com/lib/3/serial.js"></script>
    <script src="https://www.amcharts.com/lib/3/plugins/export/export.min.js"></script>
    <script src="https://www.amcharts.com/lib/3/themes/light.js"></script>
    <!-- all line chart activation -->
    <script src="<?php echo $js ?>line-chart.js"></script>
    <!-- all bar chart activation -->
    <script src="<?php echo $js ?>bar-chart.js"></script>
    <!-- all pie chart -->
    <script src="<?php echo $js ?>pie-chart.js"></script>
    <!-- others plugins -->
        <script src="<?php echo $js ?>plugins.js"></script>
        <script src="<?php echo $js ?>bootstrap.min.js"></script>
		<script src="<?php echo $js ?>metisMenu.min.js"></script>
    <!-- DataTables JavaScript -->
        <script src="<?php echo $js ?>dataTables/jquery.dataTables.min.js"></script>
        <script src="<?php echo $js ?>dataTables/dataTables.bootstrap.min.js"></script>
        <script src="<?php echo $js ?>scripts.js"></script>
	    <script src="<?php echo $js ?>startmin.js"></script>
        <script src="<?php echo $js ?>modernizr-2.8.3.min.js"></script>
             <script src="<?php echo $js ?>select2.min.js"></script>
		<script src="<?php echo $js ?>front.js"></script>
	</body>
</html>